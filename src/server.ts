...
export default function startServer(ctx: AppContext): { close: () => void } {
	...
	app.post('/map', async (c) => {
		...
		if (
			...
			return c.json(
				{ error: 'Each item must include a non-empty symbol string' },
				400,
			)
		}

		const mapped = payload.map((alias) =>
			stripAliases(mapAliasToAsset(ctx, alias)),
		)
	...
	log.info(
		`listening on ${ctx.config.server.address}:${ctx.config.server.port}`,
	)
...
function stripAliases(
	asset: Caip19Asset | null,
):
	| (Omit<Caip19Asset, 'aliases' | 'chainAliases' | 'chain'> & {
			chain: Omit<Caip19Asset['chain'], 'aliases'>
	  })
	| null {
	if (!asset) return null
	const {
		aliases: _aliases,
		chainAliases: _chainAliases,
		chain,
		...rest
	} = asset
